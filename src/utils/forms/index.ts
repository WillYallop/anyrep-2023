...
import EnhancedFileInput from "./classes/enhanced-file-input";
// Util
...
export default class FormHandler {
  ...
  #validateInput(input?: InputElement): InputValidity | undefined {
    ...
    // check if name has [] at the end
    const isGrouped = input.name.slice(-2) === "[]";
    // if grouped run group validation
    if (isGrouped) return this.#validationGroupInput(input);
  ...
  #validationGroupInput(input: InputElement): InputValidity | undefined {
    // reset inputs custom error validity
    input.setCustomValidity("");
    // runs inputs custom validation and sets custom validity
    this.#addCustomValidation(input);
    ...
    const inputs = this.form.querySelectorAll(
      `[name="${input.name}"]`
    ) as NodeListOf<InputElement>;

    // check if any pass validation
    const valid = [...inputs].some((input) => input.checkValidity());
    const name = input.name;
    const messages = [];
    if (!valid) messages.push(input.validationMessage);
    // set error class and message
  ...
  #validateForm(form: HTMLFormElement) {
    ...
    const formValid = validity.every((input) => input.valid);
    return {
      inputs: validity,
      valid: formValid,
    };
  ...
  #scrollToError() {
    if (this.config.scrollToError?.enabled) {
      const input = this.form.querySelector(
        '[aria-invalid="true"]'
      ) as InputElement;
      if (input) {
        input.focus();
        const offset = this.config.scrollToError.offset || 0;
        window.scrollTo({
          top: input.offsetTop - offset,
          behavior: "smooth",
        });
      }
    }
...
export { FlashMessage, Turnstile, GoogleV2, EnhancedFileInput };
